...
if (!sObjectIsCustomShaped && !flags['confirm-ownership']) {
    console.error(`ARCHITECTURAL GUARDRAIL: ${sObjectName} is a standard SObject. A Selector may be created only in the SObject's owning package.`);
    console.error(`Resolve ownership first (project context file's AEP Conventions section + org discovery). If the owner is another package, extend it via Selector Method Injection instead. If the developer confirms THIS project owns ${sObjectName}, re-run with --confirm-ownership.`);
    process.exit(1);
}
if (sObjectPrefix && appPrefix && sObjectPrefix !== appPrefix) {
    ...
    console.error(`Do not create a local Selector for it. Extend the owning package's selector via Selector Method Injection instead.`);
    process.exit(1);
}
